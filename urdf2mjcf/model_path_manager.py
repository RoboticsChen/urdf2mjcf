...
Examples:
  ...
  %(prog)s unset
    ...
    scan_parser = subparsers.add_parser(
        "scan",
        help="Scan directories for ROS description packages and add to URDF2MJCF_MODEL_PATH"
    )
    scan_parser.add_argument(
        "paths",
        type=str,
        nargs="+",
        help="Root directories to scan for description packages"
    )
    scan_parser.add_argument(
        "--no-append",
        action="store_true",
        help="Replace existing paths instead of appending"
    )
    scan_parser.add_argument(
        "--max-depth",
        type=int,
        default=10,
        help="Maximum recursion depth for scanning (default: 10)"
    )
    
    # List command
    list_parser = subparsers.add_parser(
        "list",
        help="List current paths in URDF2MJCF_MODEL_PATH"
    )
    
    # Unset command
    unset_parser = subparsers.add_parser(
        "unset",
        help="Unset URDF2MJCF_MODEL_PATH environment variable"
    )
